...
/// Shows nothing when count is 0 (but may show green dot if update available).
pub fn update_tray_segment_count(app: &tauri::AppHandle, count: usize) {
    if let Some(tray) = app.tray_by_id("main") {
        if count > 0 {
            ...
            // When segment count clears, show green dot if update is available
            let state = app.state::<Arc<AppState>>();
            let has_update = state.latest_release.lock().is_some();
            if has_update {
                let dot = "\u{25CF}"; // ●
                let _ = tray.set_title(Some(dot));
                #[cfg(target_os = "macos")]
                style_tray_title_green(dot);
            } else {
...
/// Show update-available indicators: green dot on tray icon + menu item.
pub fn show_update_available(app: &tauri::AppHandle, release: &crate::update::LatestRelease) {
    // Show green dot if no segment count is currently displayed
    let state = app.state::<Arc<AppState>>();
    ...
    // Add "Update Available" menu item to tray menu
...
/// Dispatch styled green tray title update to the main thread.
#[cfg(target_os = "macos")]
fn style_tray_title_green(title: &str) {
    let title_owned = title.to_string();
    let boxed = Box::into_raw(Box::new(title_owned));
    ...
    extern "C" fn do_style(ctx: *mut std::ffi::c_void) {
        unsafe {
            let title = Box::from_raw(ctx as *mut String);
            style_tray_title_green_main_thread(&title);
        }
    }
...
/// Must be called on the main thread. Sets green-colored attributed title.
#[cfg(target_os = "macos")]
unsafe fn style_tray_title_green_main_thread(title: &str) {
    use cocoa::base::{id, nil};
    use cocoa::foundation::NSString;
    use objc::runtime::Class;
    use objc::{msg_send, sel, sel_impl};
    ...
    let font: id = msg_send![
        Class::get("NSFont").unwrap(),
        monospacedDigitSystemFontOfSize: 5.0_f64
        weight: 0.4_f64
    ];
    let green: id = msg_send![Class::get("NSColor").unwrap(), systemGreenColor];
    let baseline_offset: id = msg_send![Class::get("NSNumber").unwrap(), numberWithDouble: 3.0_f64];

    let font_key = NSString::alloc(nil).init_str("NSFont");
    let color_key = NSString::alloc(nil).init_str("NSColor");
    let baseline_key = NSString::alloc(nil).init_str("NSBaselineOffset");
    let keys = [font_key, color_key, baseline_key];
    let vals = [font, green, baseline_offset];
    ...
    for i in 0..win_count {
        ...
        if name == "NSStatusBarWindow" {
            ...
            // Match any existing title (green dot or segment count)
            let _: () = msg_send![content_view, setAttributedTitle: attr_str];
            return;
        }
    }
